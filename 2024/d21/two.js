...
import {memo, range} from "../lib/utils.js";
...
function human() {
    return memo(function (outcome, target) {
...
const source = [
    ["0", numericKeypad], // <- the final bot
    ...(range(25, 1).map(x => [x.toString(), arrowKeypad])), // <- the intermediates
]

const chain = source.reduceRight(
    (next, [name, board]) => {
        return robot(name, board, next);
    }, human() // <- the first human
)
